...
namespace Books.Persistence
{
    public class UnitOfWork : IUnitOfWork
    {
        ...
        private void ValidateEntity(object entity)
        {
            ...
            {
                foreach (var b in _dbContext.Books.Where(b=>b.Publisher_Id==book.Publisher_Id).ToList())
                {                  
                        if (book.Title.Equals(b.Title))
                        {
                            throw new ValidationException(
                                new ValidationResult("Titel beim Verlag bereits vorhanden",new List<string>() { nameof(Book.Title)}),null,null
                                );
                        }                 
                }
            }        
